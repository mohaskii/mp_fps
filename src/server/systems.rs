use bevy::{
    ...
    log::info, math::Quat, prelude::EventWriter,
};
use mp_fps::{ClientMessage, PlayerAttributes};
use renet::{DefaultChannel, RenetServer, ServerEvent};

use crate::{event::ShootEvent, resources::PlayerLobby, SERVER_ADDR};
...
pub  fn handle_shoot_event_system( mut server: ResMut<RenetServer>, mut shoot_events: EventReader<ShootEvent>) {
    
    let event = shoot_events.read().next();
    if let Some(event) = event {
        let message = bincode::serialize(&mp_fps::ServerMessage::Shoot(event.0, event.1.clone())).unwrap();
        server.broadcast_message(DefaultChannel::ReliableOrdered, message);
    }
}
